...
package com.github.maxim5.snippets.netty.websockets;
...
public final class HttpHeaderUtil {
    ...
    public static boolean isKeepAlive(HttpMessage message) {
        ...
        if (connection != null && AsciiString.contentEqualsIgnoreCase(Values.CLOSE, connection)) {
            return false;
        }

        if (message.protocolVersion().isKeepAliveDefault()) {
            return !AsciiString.contentEqualsIgnoreCase(Values.CLOSE, connection);
        } else {
            return AsciiString.contentEqualsIgnoreCase(Values.KEEP_ALIVE, connection);
        }
    }
    ...
    public static boolean is100ContinueExpected(HttpMessage message) {
        ...
        if (AsciiString.contentEqualsIgnoreCase(Values.CONTINUE, value)) {
            return true;
        }
    ...
    public static void setTransferEncodingChunked(HttpMessage m, boolean chunked) {
        if (chunked) {
            ...
            while (valuesIt.hasNext()) {
                ...
                if (AsciiString.contentEqualsIgnoreCase(value, Values.CHUNKED)) {
                    valuesIt.remove();
                }
            }
            if (values.isEmpty()) {
